...
def run(**context):
    ...
    # Read all silver parquet files, excluding H71 - it only reports
    # timestamp + Wert (avg), no min/max, so it doesn't fit the
    # regular fact_measurement schema and gets its own table below.
    con.execute(f"""
        CREATE OR REPLACE VIEW total_energy_data AS
        SELECT *
        FROM read_parquet(
    ...
    # Hall H71 only has timestamp + Wert (avg) readings, no min/max,
    # so it doesn't fit the regular fact_measurement schema/star
    # schema above. It gets its own dedicated fact table, and its
    # meter info is folded into dim_meter alongside the rest.
    h71_available = True
    try:
        con.execute(f"""
            CREATE OR REPLACE VIEW h71_data AS
            SELECT *
            FROM read_parquet('s3://{SILVER_BUCKET}/H71/data.parquet')
        """)
        h71_row_count = con.execute("SELECT COUNT(*) FROM h71_data").fetchone()[0]
        print(f"Loaded {h71_row_count} rows for hall H71.")
    except duckdb.IOException:
        h71_available = False
        print("No silver data found for hall H71 - skipping H71 tables.")
    ...
    if h71_available:
        con.execute("""
            CREATE OR REPLACE TABLE fact_measurement_h71 AS
            SELECT
                hall_id,
                meter_id,
                timestamp,
            ...
            FROM h71_data
        """)
        tables.append("fact_measurement_h71")
